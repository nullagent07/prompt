...
docker exec -it vllm_llm sh -lc "grep -i ':1f40' /proc/net/tcp /proc/net/tcp6 || echo 'NO LISTEN on 8000'"


1f40 = hex(8000)

если пусто → порт 8000 реально не поднят.

Можно ещё посмотреть, какие порты вообще слушаются:

docker exec -it vllm_llm sh -lc "awk 'NR>1{print \$2}' /proc/net/tcp /proc/net/tcp6 2>/dev/null | cut -d: -f2 | sort -u | head"
